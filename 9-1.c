...
void prrnk(double ranks[RANKING_SIZE]) {
	int howeq = 0;
...
double caluc(double ranks[RANKING_SIZE], double avtry)
{
    double tmprk[RANKING_SIZE];
    int rank;
    //ランクの計算
    for (int i = 0; i < RANKING_SIZE + 1; i++){
        ...
        if (avtry <= ranks[i]){
            ...
            for (int j = i+1; j < RANKING_SIZE + 1; j++){
                tmprk[j] = ranks[j-1];
            }
...
int main(void)
{
	...
	double ranks[RANKING_SIZE] = {1.1,1.5,1.6,1.7,1.75,1.8,2.1,2.3,2.5,2.9};
